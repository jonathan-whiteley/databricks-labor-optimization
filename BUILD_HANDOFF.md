# Labor Optimization — Build Handoff

Session date: 2026-05-06 / resumed 2026-05-12. Status: **End-to-end demoable. All 5 endpoints green. Tagged v0.1.0-demoable.**

App is deployed and running at: **https://labor-optimization-planner-66306676349647.aws.databricksapps.com**
...
1. The App SP role exists in Lakebase (Postgres role `4cfbd6e8-bb5f-4aca-a611-f60365c7ca48`) but had **zero grants** on the `labor` schema or its tables. Postgres reports "does not exist" instead of "permission denied" when the role can't see a schema.
2. The native `schedules` table was created without a schema qualifier in `notebooks/05_init_schedules_table.sql`, so it landed in `public`. The app's `search_path` is just `labor`, which would have broken POST /api/schedule/save with the same symptom once stores started working.

Applied out-of-band:
- `ALTER TABLE public.schedules SET SCHEMA labor` (the 1-row smoke insert moved over with it)
- `GRANT USAGE ON SCHEMA labor TO "4cfbd6e8-..."`
- `GRANT SELECT` on the four `*_synced` tables
- `GRANT SELECT, INSERT, UPDATE, DELETE ON labor.schedules` and `USAGE, SELECT ON SEQUENCE labor.schedules_schedule_id_seq`
...
- `notebooks/05_init_schedules_table.sql` now creates `labor.schedules` explicitly and documents the four `GRANT` statements the App SP needs in Postgres (since DAB has no resource type for Lakebase grants).

## TL;DR — finish the build

```bash
cd ~/Desktop/Projects/labor-optimization
...
databricks apps deploy labor-optimization-planner \
  --source-code-path /Workspace/Users/jonathan.whiteley@databricks.com/.bundle/labor-optimization/default/files \
  --auto-approve
...
curl -s "https://labor-optimization-planner-66306676349647.aws.databricksapps.com/api/stores" -H "Authorization: Bearer $TOKEN" | head -c 500
...
- **Spec:** `~/Desktop/Vault/Work/docs/superpowers/specs/2026-05-06-labor-optimization-design.md`
- **Plan:** `~/Desktop/Vault/Work/docs/superpowers/plans/2026-05-06-labor-optimization.md`

## What's deployed and working

| Component | Resource | State |
|---|---|---|
| Repo | `~/Desktop/Projects/labor-optimization/` | 9 commits, clean WIP |
| UC schema | `jdub_demo.labor_optimization` | Has all data |
...
| Pyfunc model | `jdub_demo.labor_optimization.labor_rec_model` v5 | READY |
| Model Serving endpoint | `dev_jonathan_whiteley_labor-rec-v1` | READY, scale-to-zero enabled |
| Lakebase Autoscaling instance | `labor-optimization-db` | AVAILABLE, CU_1 |
| Lakebase host | `ep-icy-star-d1twxm2q.database.us-west-2.cloud.databricks.com` | Reachable |
| Lakebase synced reads | `labor.{sales_forecasts_synced, labor_recommendations_synced, staffing_targets_synced, stores_synced}` | All SYNCED |
| Lakebase native | `labor.schedules` | Working, smoke INSERT verified |
| Lakeflow refresh job | `labor_optimization_refresh` (DAB-managed) | Tested end-to-end, 94s runtime |
| FastAPI backend | 5 routers, 6 unit tests pass | Mounts UI static |
| React frontend | Today + Adjust + Approve, themed UI, number tweens | Built into `src/labor_iq/static/` |
| Databricks App | `labor-optimization-planner` | RUNNING |
...
Root cause: `src/labor_labor/lakebase.py` was minting an OAuth credential as the App's service principal (correct), but then connecting to Postgres with `user=jonathan.whiteley@databricks.com` from `USER_EMAIL` env (wrong). Lakebase rejects when the token's identity doesn't match the connection user.
...
- `src/labor_labor/lakebase.py` — `_db_user()` helper prefers `DATABRICKS_CLIENT_ID`
- `resources/app.yml` — endpoint reference now interpolates `${resources.model_serving_endpoints.labor_labor_endpoint.name}` (was failing because var name didn't have the dev prefix)
...
If anything still 500s, check the app logs: `databricks apps logs labor-optimization-planner`.
...
That interpolation produces a display string with a space ("app-4hg19f labor-optimization-planner") which UC rejects when used in `GRANT ... TO ...`. The first attempt to declare schema grants via the bundle failed: `cannot create grants: Could not find principal with name app-4hg19f labor-optimization-planner.`
...
```sql
GRANT USE SCHEMA, SELECT ON SCHEMA jdub_demo.labor_optimization TO `<app_sp_client_id_uuid>`;
```
...
The endpoint declared as `name: ${var.endpoint_name}` with `var.endpoint_name=labor-rec-v1` deploys as `dev_jonathan_whiteley_labor-rec-v1` (prefixed). But Lakebase declared the same way (`name: ${var.lakebase_instance}`) stays as `labor-optimization-db` (literal). The difference comes from how Terraform handles named resources of each type.

**Implication:** Cross-resource references should use `${resources.X.Y.name}` interpolation, not the var directly, so the dev prefix flows automatically. The `app.yaml` runtime env vars and any standalone code paths (e.g., the driver notebook) need to hardcode the deployed name (`dev_jonathan_whiteley_labor-rec-v1`) until production target prefixes drop.
...
```sql
ALTER TABLE jdub_demo.labor_optimization.sales_forecasts SET TBLPROPERTIES (delta.enableChangeDataFeed = true);
...
### 7. Row filter on `jdub_demo.labor_optimization.stores` was incompatible with online MV sync
...
**Heads-up:** If you ever need the row filter restored (e.g., real-user RLS for store territory boundaries), the synced table must be deleted first or a different masking strategy adopted (column-level masks on the SP's read).
...
`ui/vite.config.ts` sets `build.outDir: "../src/labor_labor/static"`. FastAPI mounts that dir last (after `/api/*` routes) with `StaticFiles(html=True)`. The mount is gated `if _static_dir.exists()` so unit tests don't fail when the UI hasn't been built.

### 10. Pydantic v2 reserves `schema` as a BaseModel attribute
The plan named the env-driven settings field `schema`. Pydantic v2 complains. Renamed to `brand_schema` in `src/labor_labor/config.py` (no other code referenced it).
...
- [ ] **App SP needs `MODIFY` on `jdub_demo.labor_optimization.schedules`** if you ever wire the outbound Lakebase→UC sync. Currently the App writes to Lakebase only, so this isn't blocking.
...
- The `stores` row filter was dropped to enable the sync (learning #7) — restore strategy needed if governance requires it

## Files of interest

```
~/Desktop/Projects/labor-optimization/
...
│   ├── public/labor-logo.svg             # From ~/Desktop/logos/
│   └── src/
