...
class Settings(BaseModel):
    ...
    brand_schema: str = os.getenv("BRAND_SCHEMA", "labor_optimization")
    lakebase_host: str = os.getenv(
        "LAKEBASE_HOST",
        "ep-icy-star-d1twxm2q.database.us-west-2.cloud.databricks.com",
    )
    lakebase_database: str = os.getenv("LAKEBASE_DATABASE", "labor_optimization")
    lakebase_pg_schema: str = os.getenv("LAKEBASE_PG_SCHEMA", "labor")
    lakebase_instance: str = os.getenv("LAKEBASE_INSTANCE_NAME", "labor-optimization-db")
    serving_endpoint: str = os.getenv(
        "SERVING_ENDPOINT_NAME", "dev_jonathan_whiteley_labor-rec-v1"
    )
